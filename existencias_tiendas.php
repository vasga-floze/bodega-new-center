...
if($_POST)
{
	...
	if($bodega=='')
	{
		$c=$conexion2->query("select codigo,COUNT(codigo) as cantidad,SUM(isnull(lbs,0)+isnull(peso,0)) as peso,bodega from registro where (fecha_desglose='' or fecha_desglose is null) and activo is null and bodega not like '0%' and tipo!='C1' group by codigo,bodega order by codigo
")or die($conexion2->error());
	}else
	{
		$c=$conexion2->query("select codigo,COUNT(codigo) as cantidad,SUM(isnull(lbs,0)+isnull(peso,0)) as peso,bodega from registro where (fecha_desglose='' or fecha_desglose is null) and activo is null and bodega='$bodega' and tipo!='C1' group by codigo,bodega order by codigo
")or die($conexion2->error());
