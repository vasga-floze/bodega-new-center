...
if($_POST)
{
	...
	$user=$_SESSION['usuario'];
		$cveri=$conexion2->query("select * from traslado where registro='$id' and sessiones='$doc' and usuario='$user'")or die($conexion2->error);
		...
		}else
		{
			...
			}else
			{
				...
				{
					$docs=$_SESSION['doc'];
					$k=$conexion2->query("select max(id) as idd from traslado where sessiones='$docs' and usuario='$usuario'")or die($conexion2-error());
			...
			{
				$qv=$conexion2->query("select * from traslado where usuario!='$usuario' and documento_inv='- -'and registro='$id'")or die($conexion2->error());
				$nqv=$qv->rowCount();
				if($nqv==0)
				{
		...
		{
			echo "<script>alert('CODIGO DE BARRA ESTA SIENDO UTILIZADO EN OTRO TRASLADO COMUNIQUENCE PLP')</script>";
			echo "<script>location.replace('traslados.php?bcod=$bodeg&&i=2')</script>";
		}
