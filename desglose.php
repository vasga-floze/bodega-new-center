...
<head>
	<?php
		...
		$qdb=$conexion1->query("select * from usuariobodega where (usuario like '%ahua%' or usuario like '%usu%') and usuario='$usu'")or die($conexion1->error());
		$nqbd=$qdb->rowCount();
			if($nqbd!=0)
			{
...
<form method="POST">
	...
	<select name="bodega_desglose" required class="text" style="width: 30%;">
	<?php
	if($_SESSION['usuario']=='staana3')
	{
	?>
		<option value="">BODEGA DESGLOSE</option>
	<?php
	$cb=$conexion1->query("select * from consny.bodega where nombre not like '%(N)%' and bodega not like 'SM%' order by nombre")or die($conexion1->error());
	while($fcb=$cb->FETCH(PDO::FETCH_ASSOC))
	{
		$bodega=$fcb['BODEGA'];
		$nombre_bodega=$fcb['NOMBRE'];
		echo "<option value='$bodega'>$bodega: $nombre_bodega</option>";
	}
	}else
	{
		$bod_usu=$_SESSION['bodega'];
		$qb=$conexion1->query("select * from consny.bodega where bodega='$bod_usu'")or die($conexion1->error());
		$fqb=$qb->FETCH(PDO::FETCH_ASSOC);
		$nombre_bodega=$fqb['NOMBRE'];
		$bodega=$fqb['BODEGA'];
		echo "<option value='$bodega'>$bodega: $nombre_bodega</option>";
	}
	?>
...
<table class="tabla" border="0" cellpadding="10" id='info'>
	...
	<td colspan="3">
		...
		<form method="POST" name="form">
			...
			<input type="number" name="cant" id="cant" class="text" placeholder="cantidad" style="width: 10%;" min="1">
...
if($_POST)
{
	...
	$c=$conexion2->query("select * from dbo.registro where barra='$barra' and bodega!='us01'")or die($conexion2->error);
	$n=$c->rowCount();
	if($n==0)
	{
		echo "<script>alert('NO SE ENCONTRO NINGUN REGISTRO DE: $barra O LA BODEGA NO ES VALIDA PARA DESGLOSE')</script>";
	}else
	{
...
echo "<script>alert('NO PUEDES DESGLOSAR ESTE FARDO NO HA SIDO ASIGNADO A LA BODEGA($bodega1v)\\n BODEGA ACTUAL DEL FARDO ES: $bodegav')</script>";
			$_SESSION['registro']='';
			echo "<script>location.replace('desglose.php')</script>";
...
$barra=$fco['barra'];
$und=$fco['und'];
$fecha_traslado=$fco['fecha_traslado'];
...
?>
<table border="1" style="float: left; width: 49%; margin-right: 0.5%; border-collapse: collapse;" cellpadding="10">
	<tr style="text-align: center;">
		<td colspan="4" style="color:white; background-color: rgb(133,133,133,0.8);">DESGLOSE PRODUCCION<?php echo "(UNIDADES: $und)";?></td>
	</tr>
	<tr>
			<td>ARTICULO</td>
			<td>DESCRIPCION</td>
	...
	<?php
	$id_registros=$_SESSION['registro'];
	$con_detalle=$conexion2->query("select * from detalle where registro='$id_registros'")or die($conexion2->error());
	$n_con_registros=$con_detalle->rowCount();
	if($n_con_registros==0)
	{
		echo "<tr style='text-align:center;'>
			<td colspan='4'>NO SE AGREGO DESGLOSE DE PRODUCCION</td>
		</tr>";
	}else
	{
		$total_pro=0;
		while($f_con_detalle=$con_detalle->FETCH(PDO::FETCH_ASSOC))
		{
			$art_pro=$f_con_detalle['articulo'];
			$cant_pro=$f_con_detalle['cantidad'];
			$id_detalle=$f_con_detalle['id'];
			$ca_pro=$conexion1->query("select * from consny.articulo where articulo='$art_pro'")or die($conexion1->error());
			$f_ca_pro=$ca_pro->FETCH(PDO::FETCH_ASSOC);
			$arti_pro=$f_ca_pro['ARTICULO'];
			$desc_pro=$f_ca_pro['DESCRIPCION'];
			echo "<tr>
			<td>$arti_pro</td>
			<td>$desc_pro</td>
			<td>$cant_pro</td>
			<td>
			<form method='POST' action='add_detalle_a_desglose.php'>
			<input type='hidden' name='id_detalle' value='$id_detalle'>
			<button style='background-color:white; padding:5%; border-color:#000; cursor:pointer;'>AGREGAR >></button></form>
			</td>
			</tr>";	
			$total_pro=$total_pro + $cant_pro;
	...
	<table border="1" class="tabla" cellpadding="10" style="text-align: center; width: 50%; float: right;">
	<?php
	}else
	{
		echo "<table border='1' class='tabla' cellpadding='10' style='text-align: center;'>";
	}
	?>
...
<tr>
	<td colspan="4">
		...
		<input type="text" name="desglosado" placeholder="DESGLOSADO POR:" required>
	<input type="text" name="dig" required id="dig" placeholder="DIGITADO POR:">
	FECHA: <input type="date" onchange="ver()" name="fecha" value='<?php echo $hoy; ?>' min='<?php echo $fecha_traslado; ?>' id='hc' required>
...
if($_POST)
{
	...
	if($btn=="Finalizar" and $kp==1 and $_SESSION['registro']!='')
	{
		...
		$cc=$conexion2->query("select * from registro where (fecha_desglose='' or fecha_desglose is null) and id_registro='$idr'")or die($conexion2->error);
		$ncc=$cc->rowCount();
		
		if($ncc==0 or $idr=="")
		{
			echo "<script>alert('SE PRODUJO UN ERROR INTENTELO NUEVAMENTE O VERIFIQUE SI YA FUE DESGLOSADO ANTES, COMUNIQUESE INMEDIATAMENTE CON OSMIN CAMPOS')</script>";
			echo "<script>location.replace('desglose.php')</script>";
		}
...
$idr=$_SESSION['registro'];
$consu_bode=$conexion2->query("select * from registro where id_registro='$idr'")or die($conexion2->error());
$fconsu_bode=$consu_bode->FETCH(PDO::FETCH_ASSOC);
$bode=$fconsu_bode['bodega'];
...
$conexion2->query("update dbo.registro set digita_desglose='$dig',fecha_desglose='$fecha',documento_inv_consumo='$ec',documento_inv_ing='$ei',desglosado_por='$desglosado' where id_registro='$idr'")or die($conexion2->error);
 ...
 // agregar precio_regular a los desgglose
	...
	//ENVIO DE CORREO
	$qc=$conexion2->query("select 'DESGLOSE' AS transaccion,registro.barra,concat(EXIMP600.consny.articulo.articulo,': ',
EXIMP600.consny.articulo.descripcion) as art,SUM(ISNULL(lbs,0)+isnull(peso,0)) as peso,registro.bodega,registro.fecha_desglose from 
registro inner join EXIMP600.consny.articulo on EXIMP600.consny.ARTICULO.ARTICULO=registro.codigo where 
registro.id_registro='$idr' group by registro.barra,concat(EXIMP600.consny.articulo.articulo,': ',EXIMP600.consny.articulo.descripcion),
registro.bodega,registro.fecha_desglose")or die($conexion2->error());

	$qc1=$conexion2->query("select CONCAT(EXIMP600.consny.ARTICULO.ARTICULO,': ',EXIMP600.consny.ARTICULO.DESCRIPCION) as articulo,
...
desglose.registro in(select id_registro from registro where id_registro='$idr')")or die($conexion2->error());
	$fqc=$qc->FETCH(PDO::FETCH_ASSOC);
	$artc=$fqc['art'];
	$bodegac=$fqc['bodega'];
	$pesoc=$fqc['peso'];
	$barrac=$fqc['barra'];
	$fecha_desglose1=$fqc['fecha_desglose'];
...
$tabla.="<tr>
	...
	<td>$artc</td>
	<td>$pesoc</td>
	<td>$fecha_desglose1</td>
<tr></table><hr>";
$tabla.="<table border='1' cellpadding='10' style='border-collapse:collapse;'>";
$tabla.="<tr>
	<td colspan='4'>DETALLE DESGLOSE</td>
</tr>";
$tabla.="<tr>
	<td>ARTICULO</td>
	<td>CANTIDAD</td>
...
$tcantidad=0;
$ttotal=0;
while($fqc1=$qc1->FETCH(PDO::FETCH_ASSOC))
{
	$artd=$fqc1['articulo'];
	$cantd=$fqc1['cantidad'];
	$precio=$fqc1['precio'];
...
$asunto="*SISTEMA* DESGLOSE $bodegac BARRA: $barra";
