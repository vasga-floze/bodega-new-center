 	...
 	if($origenes==$codb)
 	{
 		...
 		}else
 		{
 			...
 			}else
 			{
 				...
 				$sessiones_doc=$_SESSION['doc'];
 				$user=$_SESSION['usuario'];
 				//echo "<script>alert('update $codb $sessiones_doc $user')</script>";
 				$conexion2->query("update  traslado set destino='$codb' where sessiones='$sessiones_doc' and usuario='$user'")or die($conexion2->error());
 				echo "<script>location.replace('traslados.php?i=2&&bcod=$codb')</script>";
 			}
