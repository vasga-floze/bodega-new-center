...
{
	echo "<script>alert('NO TIENES PERMISO PARA HACER TRASLADOS')</script>";
	echo "<script>location.replace('conexiones.php')</script>";
}
if($_SESSION['traslado']=='' and $_SESSION['usuario']!='OCAMPOS')
{
	$usuv=strtoupper($_SESSION['usuario']);
	$qvc=$conexion2->query("select * from traslado where usuario='$usuv' and estado='0'")or die($conexion2->errror());
	$nqvc=$qvc->rowCount();
	if($nqvc!=0)
	{
		echo "<script>alert('ALGUIEN MAS ESTA USANDO TU USUARIO EN OTRO TRASLADO BUSCALO EN PENDIENTES Y FINALIZALO PRIMERO O ABRISTE OTRA VENTANA, YA SE LES DIJO QUE NO LO PUEDEN HACER PLGP!!!')</script>";
		echo "<script>location.replace('salir.php')</script>";
	}
}
...
group by origen,destino,sessiones,usuario,paquete,articulo")or die($conexion2->error());
		...
		}else
		{
			...
			$usuf=strtoupper($_SESSION['usuario']);
			$docf=$_SESSION['traslado'];
			$cts=$conexion2->query("select  registro,'INGRESO POR TRASLADO DE FARDOS' referencia,'E' tipo,
articulo,1 cantidad, case registro.tipo when 'P' then lbs when 'CD' then
peso end peso,fecha,traslado.usuario,traslado.paquete,traslado.destino bodega,getdate() fechai,'traslado' tabla,
traslado.documento_inv from traslado inner join registro on traslado.registro=
registro.id_registro where traslado.sessiones='$docf' and traslado.usuario='$usuf' and traslado.estado='1'
union all

select  registro,'SALIDA POR TRASLADO DE FARDOS' referencia,'S' tipo,
articulo,-1 cantidad, case registro.tipo when 'P' then lbs when 'CD' then
peso end *-1 peso,fecha,traslado.usuario,traslado.paquete,traslado.origen,getdate() fechai,'traslado' tabla,
traslado.documento_inv from traslado inner join registro on traslado.registro=
registro.id_registro where traslado.sessiones='$docf' and traslado.usuario='$usuf' and traslado.estado='1'
")or die($conexion2->error());
			while($fcts=$cts->FETCH(PDO::FETCH_ASSOC))
			{
				$registro=$fcts['registro'];
				$referencia=$fcts['referencia'];
				$tipo=$fcts['tipo'];
				$articulo=$fcts['articulo'];
				$cantidad=$fcts['cantidad'];
				$peso=$fcts['peso'];
				$fecha=$fcts['fecha'];
				$usuario=$fcts['usuario'];
				$paquete=$fcts['paquete'];
				$bodega=$fcts['bodega'];
				$fechaing=$fcts['fechai'];
				$tabla=$fcts['tabla'];
				$documentos=$fcts['documento_inv'];
				$conexion2->query("insert into transaccion_sys(registro,referencia,tipo_transaccion,
					articulo,cantidad,peso,fecha_documento,usuario,paquete,bodega,fecha_creacion,tabla,documento_inv) values('$registro','$referencia','$tipo','$articulo','$cantidad','$peso','$fecha','$usuario','$paquete','$bodega','$fechaing','$tabla','$documentos')")or die($conexion2->error());
			}
