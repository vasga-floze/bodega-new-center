...
$query=$dbEximp600->prepare(
                ...
                FROM USUARIOBODEGA 
                INNER JOIN
                    ".$respuesta.".BODEGA ON USUARIOBODEGA.BODEGA = 
...
$queryValidacionTransaccion=$dbBodega->prepare(
            "SELECT REGISTRO.CodigoBarra,
                ...
                INNER JOIN
                TRANSACCION 
                ON REGISTRO.CodigoBarra=TRANSACCION.CodigoBarra
                WHERE (
...
$response["message"]="
                        El codigo de barra esta siendo 
                        utilizado en otra transaccion
                    ";
$response["success"]="false";
echo(json_encode($response));
return;
...
SUM(ROUND((DETALLEREGISTRO.Cantidad*DETALLEREGISTRO.PrecioUnitario)/1.10,0)) 
VALORMINIMO,
SUM(ROUND((DETALLEREGISTRO.Cantidad*DETALLEREGISTRO.PrecioUnitario)*1.10,0)) 
VALORMAXIMO
...
$response["message"]="Algo no coincide con los registro"
...
try { 
$conexionServerHamachi = new PDO("sqlsrv:Server=".$hamachi.
                            ";Database=".$base."", "sa", 
                            "$0ftland"
                        );
$response["message"]="Conexion exitosa al servidor SQL".$articulo;
for ($i=0; $i <$numeroDatosDetalle ; $i++) { 
$queryValidarDatosAnteriores=$conexionServerHamachi->prepare(
                                    "SELECT 
                                    COUNT(".$respuesta."
                                    .EXISTENCIA_BODEGA.ARTICULO
                                    )
FROM ".$respuesta.".EXISTENCIA_BODEGA INNER JOIN
".$respuesta.".ARTICULO_PRECIO ON ".$respuesta.
".EXISTENCIA_BODEGA.ARTICULO = ".$respuesta.
".ARTICULO_PRECIO.ARTICULO
WHERE (".$respuesta.".EXISTENCIA_BODEGA.ARTICULO = '$articulo') 
    AND 
(".$respuesta.".EXISTENCIA_BODEGA.BODEGA = '$bodega')
");
...
$response["success"]="true";
    ...
    $response["success"]="false";
    $response["message"]="
                        Error en la conexion al servidor SQL:
                    ".$e->getMessage();
...
/**
 * *ESTA QUERY INSERTA LA TUPLA TRANSACCION
* @param codigoBarra contiene el valor que introduce el usuario
* @param IdTipoTransaccion contiene un valor quemado
* @param fechaActual contiene la fecha actual de la maquina
* @param bodega contiene la bodega que sale de la consulta de la tienda
* @param naturaleza contiene un valor quemado
* @param usuario contiene el usuario que se logueo
*/
...
$fechaProxima = $fechaActualFormateado->setDate(
                            $anioProximo,
                            $fechaActualFormateado->format('m'),
                            $fechaActualFormateado->format('d')
                        );
...
/**
 * TODO: ESTA QUERY 
 * TODO: SELECCIONA EL DOCUMENTO INVENTARIO DE TIPO CONSUMO
 * 
 */
...
$referencia='CONSUMO POR DESGLOSE FARDO: ' 
        .$articuloPaquete. 'CODIGO BARRA ' 
        .$codigoBarra;
...
/**
 * * ESTA QUERY INSERTA LA TRANSACCION DE INVENTARIO SALIDA
 * @param paqueteInventario contiene el paquete de la consulta de la tienda
 * @param siguienteConsec contiene el siguiente consecutivo
 * @param consumo contiene un valor quemado
 * @param referencia contiene informacion quemada
 * @param fechaActual contiene la fecha de la maquina
 * @param seleccionado contiene un valor quemado
 * @param usuario contiene el usuario logueado
 * @param mensajeSistema contiene un valor vacio quemado
 * @param aprobado contiene un valor quemado
 * @param NoteExistsFlag contiene un valor quemado
 * 
 */
...
/**
*TODO: ESTA FUNCION RETORNA EL SIGUIENTE CONSECUTIVO
*@param siguienteConsec contiene el siguiente consecutivo
*@return consecutivo el siguiente consecutivo
* 
* */
...
$queryActualizarConsecutivo="UPDATE ".$respuesta."
                                    .consecutivo_ci SET SIGUIENTE_CONSEC=? 
                                    WHERE consecutivo=?";
$queryActualizarConsecutivoEjecutar=$dbEximp600->prepare(
                                                $queryActualizarConsecutivo
                                            );
if(!$queryActualizarConsecutivoEjecutar->execute([
...
$querySiguienteConsecutivoIng=$dbEximp600->prepare(
...
foreach ($datosDetalle as $value) {
    ...
    $queryInsertarCodigoBarraLote=$dbEximp600->prepare(
                                            "INSERT INTO ".$respuesta.".LOTE
                                                (
                                                    LOTE,
                                                    ARTICULO,
                                                    LOTE_DEL_PROVEEDOR,
...
$queryPiezaFardoEncabezado=$dbEximp600->prepare(
                        "INSERT INTO ".$respuesta.".DOCUMENTO_INV
                        (
                            PAQUETE_INVENTARIO,
                            DOCUMENTO_INV,
                            CONSECUTIVO,
...
foreach ($datosDetalle as $value) {
        ...
        $queryTransaccionPiezaFardo=$dbEximp600->prepare(
                                    " INSERT INTO ".$respuesta.".LINEA_DOC_INV
                                            (
                                                PAQUETE_INVENTARIO,
                                                DOCUMENTO_INV,
                                                LINEA_DOC_INV,
...
$queryUpdateActualizarUltimoDocumento=$dbEximp600->prepare(
                                "UPDATE ".$respuesta.
                                ".CONSECUTIVO_CI 
                                SET SIGUIENTE_CONSEC=:documentoConsecutivo 
                                WHERE CONSECUTIVO='ING'
                                ");
$queryUpdateActualizarUltimoDocumento->bindParam(
        ":documentoConsecutivo",$documentoConsecutivoING
        );
